...
Versão Python do programa original em Pascal/Lazarus do SR. Guaracy
...
class PDFToEtiquetaApp:
    ...
    def processar_pdf(self):
        ...
        try:
            ...
            self.status_var.set(f"{self.total_etiquetas} Etiquetas em {elapsed_time:.2f}s")
    ...
    def gerar_etiquetas(self, pdf_filename):
        """Processa o .txt página a página (divide por formfeed)"""
        ...
        # Dividir por formfeed — cada item da lista é uma página (sem o '\x0c')
        ...
        for page_idx, page in enumerate(pages, start=1):
            # Quebrar em linhas e remover cabeças/caudas vazias
            lines = [ln for ln in page.splitlines()]
            # remover linhas vazias do topo e do fim
            ...
            if etiqueta:
                try:
                    ...
                    print(f"Erro ao criar imagem da etiqueta (página {page_idx}): {e}")
                    continue
    
    def extrair_dados_etiqueta(self, page_lines):
        """
        Extrai dados de uma única página (lista de linhas).
        Assume: primeira linha = título; procura Cód.:, Cód.Cliente:, O.S./Item: nas linhas seguintes.
        """
        if not page_lines:
            return None
        try:
            ...
            for i in range(1, len(page_lines)):
                ...
                if linha.startswith("Cód.:"):
                    ...
                    # Procurar a próxima linha não vazia que contenha o número da OS
                    found = ""
                    ...
                    if found:
                        ...
                        # talvez o próprio linha contenha o número após o label
    ...
    def criar_imagem_etiqueta(self, etiqueta):
        """Cria a imagem da etiqueta"""
        width, height = 640, 400
        img = Image.new('RGB', (width, height), 'white')
        draw = ImageDraw.Draw(img)
        
        # Tentar carregar fontes; fallback para load_default
        try:
        ...
        # Carregar logo (vários caminhos possíveis)
        ...
        for logo_path in possible_logo_paths:
            if os.path.exists(logo_path):
                try:
                    ...
                    continue
        if not logo_loaded:
            print("AVISO: Logo projelmec.png não encontrada!")
        ...
        # Código (esquerda) e Código do Cliente (direita)
        ...
        try:
            ...
            if codcli_x < margin + 100:
                # evitar sobreposição caso texto muito longo
                codcli_x = width // 2
        except Exception:
            codcli_x = width // 2
